...
class User(ndb.Model):
    guest_email = ndb.StringProperty(indexed=True)
    guest_first_name = ndb.StringProperty(indexed=True)
    guest_last_name = ndb.StringProperty(indexed=True)
    guest_phone_num = ndb.IntegerProperty()
    guest_date = ndb.DateTimeProperty(auto_now_add=True)
    guest_identity = ndb.StringProperty(indexed=False)
    guest_authority = ndb.StringProperty(indexed=False)
...
class RegisterPage(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        u = User.query(User.guest_first_name == fname).fetch()
        if u:
            for user in u:
                if user.guest_last_name == lname:
        ...
        if faculty_email:
            ...
            authority = 'reader'
        user = User(id = user_id, guest_email=email,guest_first_name=fname,guest_last_name = lname,guest_phone_num = pnum,guest_authority= authority)
    ...
    def get(self):
        ...
        for thes in thesis:
            ...
            thesis_list.append({
                ...
                'thesis_creator_fname':creator.guest_first_name,
                'thesis_creator_lname':creator.guest_last_name,
                })
    ...
    def post(self):
        ...
        if users.get_current_user():
            greeting.author = User(
                    guest_identity=users.get_current_user().user_id(),
                    guest_email=users.get_current_user().email())
